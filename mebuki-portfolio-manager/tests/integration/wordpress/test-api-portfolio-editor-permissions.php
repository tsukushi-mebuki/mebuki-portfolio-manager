...
class Test_API_Portfolio_Editor_Permissions extends WP_UnitTestCase {
	...
	public function test_portfolio_editor_can_operate_only_own_reviews() {
		...
		$delete_other = new WP_REST_Request( 'DELETE', '/mebuki-pm/v1/reviews/' . $other_review_id );
		$delete_other_response = rest_do_request( $delete_other );
		$this->assertSame( 404, $delete_other_response->get_status() );

		$delete_own = new WP_REST_Request( 'DELETE', '/mebuki-pm/v1/reviews/' . $own_review_id );
		$delete_own_response = rest_do_request( $delete_own );
		$this->assertSame( 200, $delete_own_response->get_status() );
		$del_data = $delete_own_response->get_data();
		$this->assertTrue( $del_data['success'] );
		$this->assertSame( $own_review_id, (int) $del_data['id'] );
	}
}
